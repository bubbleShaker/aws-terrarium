...
公式には式として書かれていないが、上記の制約から導出できる:

```
partitions >= ceil(RCU / 3000)
partitions >= ceil(WCU / 1000)
partitions >= ceil(sizeGB / 10)     ← 1-3 の注記つき
```

加えて、**一度分割したパーティションは元に戻らない**（スループットを下げても減らない）。
これは「一時的にプロビジョニングを上げると後で 1 パーティションあたりの取り分が減る」という
古典的な落とし穴の原因であり、シミュレータで再現する価値がある。
...
ベストエフォートであり、依存すべきではない（公式もそう注意している）。

シミュレータではトークンバケットで表現する（容量 = 300 秒 × 毎秒レート）。
